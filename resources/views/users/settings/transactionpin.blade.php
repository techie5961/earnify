...
@section('main')
    <section class="w-full flex-auto column g-10">
        ...
        <div class="head w-full z-index-300 row align-center space-between g-10 bg-light p-15px pos-fixed top-0 right-0 left-0">
           <div onclick="Redirect('{{ url()->previous() == url()->current() ? url('users/dashboard') : url()->previous() }}')" class="glass pc-pointer h-40 w-40 no-shrink circle column align-center justify-center">
        ...
        <div class="contents w-full flex-auto column g-10 p-top-0 p-20px">
            ...
            <span class="opacity-07">Your 6-digits transaction pin secures your account from unauthorised transactions. Please, dont share it with anyone.</span>
        {{-- form --}}
        <form onsubmit="PostRequest(event,this,Completed)" action="{{ url('users/post/update/transaction/pin/process') }}" method="POST" class="w-full flex-auto column g-15px">
          {{-- csrf token --}}
          <input type="hidden" class="inp input" name="_token" value="{{ @csrf_token() }}">
            ...
            <div class="column g-4px w-full">
                ...
                <div class="cont bg-rgt-01">
                    <input name="pin[current]" type="password" inputmode="numeric" maxlength="6" placeholder="* * * * * *" class="inp input required">
                </div>
            </div>
           @endisset
             {{-- new input --}}
            <div class="column g-4px w-full">
                <span>New Pin</span>
                <div class="cont bg-rgt-01">
                    <input name="pin[new]" type="password" inputmode="numeric" maxlength="6" placeholder="* * * * * *" class="inp input required">
                </div>
            </div>
             {{-- new input --}}
            <div class="column g-4px w-full">
                ...
                <div class="cont bg-rgt-01">
                    <input name="pin[confirm]" type="password" inputmode="numeric" maxlength="6" placeholder="* * * * * *" class="inp input required">
                </div>
            </div>
            <button style="margin-top:auto;" class="post">{{ isset(Auth::guard('users')->user()->transaction_pin) ? 'Change Pin' : 'Set Pin' }}</button>
        </form>
        </div>
    </section>
@endsection
