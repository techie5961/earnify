    ...
    <section class="w-full column g-10">
        <div style="border:1px solid var(--rgt-01)" class="w-full p-20 br-10 column g-10 bg-light">
            ...
            <div class="row align-center w-full space-between g-15">
                <img style="filter:brightness(0)" src="{{ asset(config('settings.logo')) }}" alt="" class="w-70 no-shrink no-pointer no-select">
           ...
           @if ($data->type == 'withdrawal')
            ...
            <div class="row w-full align-center space-between g-10">
                ...
                <span onclick="copy('{{ json_decode($data->json)->bank->account_number ?? 'null' }}')" class="text-align-end font-weight-500 c-primary u pointer">Click to copy</span>
            </div>
           @endif
            @isset($data->data)
            ...
            @endisset
             {{-- new row --}}
            <div class="row w-full align-center space-between g-10">
                <span class="opacity-07">{{ ucwords($data->class) }}ed {{ $data->class == 'credit' ? 'to' : 'from' }}</span>
                <span class="font-weight-500 text-align-end">{{ json_decode($data->wallet)->name ?? 'Main Wallet' }}</span>
            </div>
