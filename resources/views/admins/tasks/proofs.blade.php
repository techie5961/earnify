...
@section('main')
    <section class="w-full column g-10">
        ...
        <div style="border:1px solid var(--rgt-01)" class="analytic br-primary bg-light w-full p-20 row g-10">
            ...
            <div class="column g-5">
                ...
                <strong class="font-size-1-2 font-weight-900">{{ number_format($total_pending) }}</strong>
            </div>
        </div>
         {{-- analytic --}}
        <div style="border:1px solid var(--rgt-01)" class="analytic br-primary bg-light w-full p-20 row g-10">
            ...
            <div class="column g-5">
                ...
                <strong class="font-size-1-2 font-weight-900">{{ number_format($total_approved) }}</strong>
            </div>
        </div>
        {{-- search --}}
                ...
                <div style="border:1px solid var(--rgt-01);" class="column w-full p-20 g-10 br-primary bg-light">
                    ...
                    <div style="border-bottom:1px dashed var(--rgt-01);padding-bottom:10px;" class="row align-center w-full g-10 space-between">
                        ...
                        <div class="column m-right-auto g-5">
                            <strong class="font-size-1 c-primary u">{{ ucwords($data->user->username) }}</strong>
                    ...
                    <div class="w-full row align-center g-10">
                        <span>Task Name:</span>
                        <span>{{ $data->task->title ?? 'null' }}</span>
                    </div>
                   
                     {{-- new row --}}
                     <div class="w-full row align-center g-10">
                        <span>{{ $data->status == 'success' ? 'Reward Earned' : 'Task Reward' }}:</span>
                        <div class="font-weight-900 p-5 p-x-10 br-5" style="background:rgba(0,255,0,0.1);color:#4caf50;">{{ $currency }}{{ number_format($data->task->reward,2) }}</div>
                    </div>
                    {{-- new row --}}
                    <div style="border-top:1px dashed var(--rgt-01)" class="row w-full p-top-10 align-center space-between g-10">
                      @if ($data->status == 'pending')
                          
                      <button onclick="ShowApproveModal('{{ $data->id }}','{{ $currency }}{{ number_format($data->task->reward,2) }}')" class="btn-green-3d">
    ...
    <section onclick="this.classList.remove('active')" class="modal approve">
        <div onclick="event.stopPropagation()" class="child">
            <form method="POST" action="{{ url('admins/post/approve/task/process') }}" tonsubmit="PostRequest(event,this,Completed)" class="w-full column g-10 align-center text-center">
               {{-- csrf token --}}
               <input type="hidden" class="inp input" name="_token" value="{{ @csrf_token() }}">
               {{-- task id --}}
