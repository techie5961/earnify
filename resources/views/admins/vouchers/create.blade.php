...
@section('main')
    <section x-data="{  }" class="w-full column g-10px">
        <form x-on:submit.prevent="PostRequest(event,$el,function(response){
        ...
        })" method="POST" action="{{ url('admins/post/create/voucher/process') }}" class="w-full column g-10px bg-light p-20px br-10px border-width-1px border-color-rgt-01 border-style-solid">
           {{-- csrf token --}}
           ...
           {{-- new row --}}
            <div class="row c-primary w-full align-center g-10px">
                ...
                <strong class="desc font-weight-900 c-primary no-select">Create Voucher</strong>
            </div>
            ...
            <div class="w-full column g-5px">
                ...
                <div class="cont">
                    <select name="purpose" class="inp input required">
                        ...
                        <option value="games_voucher">Games Voucher</option>
                    </select>
                </div>
            </div>
             {{-- new input --}}
            <div class="w-full column g-5px">
               <div class="column g-2px">
                ...
                <div class="cont">
                 <input name="value" type="number" inputmode="numeric" placeholder="Enter voucher value" class="inp input required">
                </div>
            </div>
            <button class="post">Create Voucher</button>
        </form>
    </section>
@endsection