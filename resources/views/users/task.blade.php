        ...
        @isset($task->banner)
            ...
            <div class="column p-top-10 w-full g-5">
                <div class="w-full row align-center g-10 space-between">
                    ...
                    <div style="background:var(--primary-01);color:var(--primary-light);border:1px solid var(--primary-light)" class="h-fit no-select br-1000 p-10 p-x-20 font-weight-400">Task Reward: {{ $currency.number_format(json_decode($task->type)->earning,2) }}</div>
                </div>
                <small class="opacity-07">Click the link to perform the task</small>
            <a style="color:var(--primary-lighter) !important;" href="{{ $task->link }}" target="_blank" class="c-primary">{{ $task->link }}</a>
            </div>
            {{-- screenshot --}}
            <form action="{{ url('users/post/task/complete/process') }}" method="POST" onsubmit="PostRequest(event,this,Completed)" class="column p-top-10 g-10 w-full">
                ...
                <img src="" alt="" class="h-full">
                 ...
                 <span style="color:coral;" class="w-full c-red text-center opacity-07">Marking a task complete when you didn't actually complete it can lead to a permanent ban of your account.</span>
            <button class="post">
